...
$systemcontext = context_system::instance();
 ...
 require_capability('block/aws_email_analytics:addinstance', $systemcontext);
	...
	if(isset($_GET['type']))
	{
    $returnurl = new moodle_url('/blocks/aws_email_analytics/list.php', array('type' => $_GET['type'] ,'sort' => $sort, 'dir' => $dir, 'perpage' => $perpage, 'page'=>$page));
	}else{
	$returnurl = new moodle_url('/blocks/aws_email_analytics/list.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage, 'page'=>$page));	
	}
    ...
    foreach ($columns as $column) {
        ...
        $$column = "<a href=\"list.php?sort=$column&amp;dir=$columndir\">".$string[$column]."</a>$columnicon";
    }
    ...
    $users = get_mail_users_listing($sort, $dir, $page*$perpage, $perpage, '', '', '',
            $extrasql, $params, $context,$status);
   ...
   if(isset($_GET['type']))
   {
    $baseurl = new moodle_url('/blocks/aws_email_analytics/list.php', array('type'=>$_GET['type'],'sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));
   }else{
	$baseurl = new moodle_url('/blocks/aws_email_analytics/list.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));   
   }
	...
	// exit;
	   ...
	   .img-set {
		   width:23%;
			...
			if($user->notificationtype == 'Delivery')
			{
				$rowStatus = "<img class='img-set' src='$CFG->wwwroot/blocks/aws_email_analytics/icons/delivered_plugin.svg'/>";
			}
			if($user->notificationtype == 'Bounce')
			{
				$rowStatus = "<img class='img-set' src='$CFG->wwwroot/blocks/aws_email_analytics/icons/bounce_plugin.svg' />";
			}
			if($user->notificationtype == 'Complaint')
			{
				$rowStatus = "<img class='img-set' src='$CFG->wwwroot/blocks/aws_email_analytics/icons/rejected_plugin.svg' />";
			}
			$row[] = $rowStatus;
